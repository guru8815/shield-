...
DROP POLICY IF EXISTS "Age verified users can create stories" ON public.stories;
DROP POLICY IF EXISTS "Age verified users can create chat rooms" ON public.chat_rooms;
...
TO authenticated
WITH CHECK (
  auth.uid() = user_id AND public.meets_age_requirement(auth.uid(), 18)
);
...
TO authenticated
WITH CHECK (
  auth.uid() = created_by AND public.meets_age_requirement(auth.uid(), 13)
);